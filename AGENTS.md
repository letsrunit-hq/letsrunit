...
packages/
  ...
  cucumber/     integration for cucumber cli
  ...
  store/        storage for run artifacts
...
1. Run the narrowest set of tests that covers the change — a single file, a single package, or all packages — and confirm they pass:
   ...
   ```
2. Update the documentation if documented behaviour is changed. Add documentation if necessary.
...
Documentation lives in `docs/` and uses **GitBook syntax**. Use the `gitbook` skill if availabe.

Write for developers who are new to letsrunit. Be concise and task-oriented: lead with what the user needs to do, not with background theory. Use active voice. Prefer examples over explanations.
