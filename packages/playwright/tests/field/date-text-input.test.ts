...
describe('setDateTextInput', () => {
  ...
  it('handles 2-digit years correctly', async () => {
    const el = createMockLocator();
    const date = new Date(2024, 0, 15);
    ...
    // Mock failure for 4-digit year, success for 2-digit
    el.inputValue.mockImplementation(async () => {
      const lastFill = el.fill.mock.calls[el.fill.mock.calls.length - 1][0];
      if (lastFill === '01/15/24') return '01/15/24';
      return '';
    });
    ...
    expect(result).toBe(true);
    expect(el.fill).toHaveBeenCalledWith('01/15/24', undefined);
  });
