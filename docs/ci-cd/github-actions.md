...
`npx letsrunit init --with-github-actions` scaffolds a workflow that installs dependencies, starts your app in the runner, waits for `LETSRUNIT_BASE_URL`, and runs `npx cucumber-js`. In an interactive terminal, `init` confirms the detected build command, start command, base URL, database service, migration command, seed command, and mailbox service before writing the workflow.
...
jobs:
  features:
    ...
    env:
      LETSRUNIT_BASE_URL: http://localhost:3000
    steps:
...
{% hint style="info" %}
New generated `cucumber.js` files read `process.env.LETSRUNIT_BASE_URL`. If you already have a Cucumber config, `init` does not patch it; add that environment fallback manually.
{% endhint %}
...
- name: Run features
  ...
  env:
    ...
    LETSRUNIT_MAILPIT_BASE_URL: http://localhost:8025
```
...
- name: Run features
  run: npx cucumber-js
  env:
    LETSRUNIT_BASE_URL: https://staging.yourapp.com
```
